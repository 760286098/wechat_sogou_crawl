        ...
        for wz_item in wz_list:
            try:
                ...
                if wz_item['type'] == '49':
                    ...
                    if content:
                        img_path = 'img\\wordcloud\\' + str(wz_item['qunfa_id']) + str(wz_item['main']) + '.png'
                        WordCloud(font_path="simkai.ttf", background_color="white", width=400, height=300, max_words=50,
                                  scale=5).generate(' '.join(jieba.cut(content))).to_file(img_path)
